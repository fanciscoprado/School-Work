		...
		stats_t *race_stats )             /* in & out */
{
    ...
    for (d=0; d < NRACERS; d++)
    {
        ...
        for ( t=0; t < TRIES; t++)
        {
            ...
            if (race_stats->fast_time > driver_list[d].tries[t])
                race_stats->fast_time = driver_list[d].tries[t];
            if (race_stats->slow_time < driver_list[d].tries[t])
...
{
    race_stats->median = 0;
    int mid_point = NRACERS/2;
    if (NRACERS % 2 != 0)
        race_stats->median = driver_list[mid_point].best_time;
    else
        race_stats->median =  (driver_list[mid_point].best_time + driver_list[mid_point - 1 ].best_time) / 2;
} 
